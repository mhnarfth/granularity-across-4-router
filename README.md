...
2.  **Architectural Validation:** Is our proposed hybrid GRU-LSTM architecture empirically superior to its standalone components (GRU-only, LSTM-only) and a strong, modern deep learning baseline (`N-HiTS`)?
...
│
└── outputs/
    ├── eda/                      # Exploratory plots (saved as PDF)
    │   └── *.pdf
    │
    ...
    └── results/
        ...
        │
        └── figures/                  # Final, paper-quality figures (saved as PDF)
            └── *.pdf
```
...
*   **Consolidated Processed Data:**
    *   After being processed by `01_ETL.ipynb`, the clean, combined dataset for this study is also stored on NRDstor to manage storage and facilitate access from compute nodes.
...
> **Objective:** To create a single, clean, and consolidated dataset from the raw network flows.
...
*   **Input:** Raw Parquet files from the NRDstor path mentioned above.
...
*   **What it Does:** This notebook loads the consolidated dataset file from Step 1 and generates high-level plots comparing the traffic volumes and daily patterns of the four routers. It provides the initial insights that motivate our study.
*   **Input:** The consolidated Parquet file from Step 1.
*   **Output:** A set of exploratory plots saved in PDF format to `outputs/eda/`.
...
*   **Experimental Space:**
    *   **Models Benchmarked:**
        1.  **`hybrid_gru_lstm`**: Our proposed hybrid model from the original paper.
        2.  **`gru_only`**: Standalone GRU model (for ablation).
        3.  **`lstm_only`**: Standalone LSTM model (for ablation).
        4.  **`nhits`**: A strong, modern MLP-based baseline (N-HiTS).
...
*   **▶️ How to Run:** Open `notebooks/03_Experiments.ipynb`. Review the configuration knobs at the top. For a full run, use all settings; for a quick test, reduce the lists. Execute all cells and monitor the progress. **This will take a significant amount of time.**
...
*   **Outputs:** The final figures for the paper (saved as PDF to `outputs/results/figures/`) and summary tables printed directly in the notebook.
